...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if(victim->frame & PG_DIRTY){
			if((swap_offset = swap_pageout(frame, victim->swap_off)) != INVALID_SWAP){
			...
			victim->frame &= ~PG_VALID; // set valid bit to 0
			victim->frame |= PG_ONSWAP; // set onswap bit to 1
			evict_clean_count++;
		}
	}
...
pgdir_entry_t init_second_level() {
	...
	pgtbl_entry_t *pgtbl; //array of page tables
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgtbl_entry_t *pgtbl = NULL;
	int frame = -1;

	// If 2nd-level page table invalid, initialize
	if(!(pgdir[idx].pde & PG_VALID)){
		pgdir[idx] = init_second_level(); 
	}

	assert(pgdir[idx].pde & PG_VALID);
	...
	if(p->frame & PG_VALID){
		...
		if(p->frame & PG_ONSWAP){
			...
			assert(!(p->frame & PG_DIRTY));
		}
		else if(!(p->frame & PG_ONSWAP)){
			init_frame(frame, vaddr); // initialize the frame
			p->frame = frame << PAGE_SHIFT;
			p->frame |= PG_DIRTY; // set dirty bit to 1
		}	
	}
